...
object Agent {
  ...
  def fromUnsafeFunction[I, O](agentName: String)(f: I => O): Agent[I, O] =
    fromFunction(agentName) { input =>
      ResultObject.safely(f(input))
    }
  ...
  def fromUnsafeFuture[I, O](agentName: String)(f: I => Future[O]): Agent[I, O] =
    fromFuture(agentName) { input =>
      f(input).map(Right(_))(scala.concurrent.ExecutionContext.global).recover { error =>
        Left(OrchestrationError.NodeExecutionError(
  ...
  def simpleFailure[I, O](agentName: String, errorMessage: String): Agent[I, O] = {
    ...
    failure(agentName, OrchestrationError.NodeExecutionError.nonRecoverable(
      agentId.value, 
      agentName, 
      errorMessage
    ))
