...
class PlanRunner {
   ...
   */
  def execute(plan: Plan, initialInputs: Map[String, Any])(implicit ec: ExecutionContext): AsyncResult[Map[String, Any]] = {
    val planId = PlanId.generate()
    
    // Set up MDC context for tracing
    MDC.put("planId", planId.value)
    MDC.put("component", "plan-runner")
    logger.info("Starting plan execution with {} nodes and {} edges", 
      plan.nodes.size, plan.edges.size)
  ...
  private def executeWithContext(plan: Plan, initialInputs: Map[String, Any], planId: PlanId)(implicit ec: ExecutionContext): AsyncResult[Map[String, Any]] = {
      ...
      case Right(_) => 
        ...
        plan.getParallelBatches match {
          case Left(error) => 
            ...
            Future.successful(Left(OrchestrationError.PlanValidationError(s"Batch creation failed: $error", planId.value)))
          case Right(batches) => 
            logger.debug("Created {} execution batches", batches.size)
            
            // Execute batches sequentially, nodes within batches in parallel
            executeBatches(batches, initialInputs, plan, planId).map {
              case Right(results) => 
              ...
              Left(OrchestrationError.PlanExecutionError.withCause(
  ...
  private def executeBatches(
    ...
    def executeBatch(
      ...
      logger.debug("Executing batch {} with {} nodes: {}", 
        batchIndex, batch.size, batch.map(_.id).mkString(", "))
      
      // Execute all nodes in this batch in parallel
      val nodeExecutions = batch.map { node =>
        executeNode(node, currentResults, plan, planId).map { result =>
          result.map(r => node.id -> r)
        }
      }
      
      // Wait for all parallel executions to complete
      Future.sequence(nodeExecutions).map { results =>
        ...
        Left(OrchestrationError.PlanExecutionError(
  ...
  private def resolveNodeInput(node: Node[_, _], availableInputs: Map[String, Any], plan: Plan): Option[Any] = {
    ...
    if (incomingEdges.isEmpty) {
      ...
      incomingEdges.collectFirst {
        case edge => availableInputs.get(edge.source.id)
      }.flatten
    }
  }

  private def executeNode(node: Node[_, _], availableInputs: Map[String, Any], plan: Plan, @annotation.unused planId: PlanId)(implicit ec: ExecutionContext): AsyncResult[Any] = {
    ...
    val resultFuture = nodeInput match {
      ...
      case None =>
        ...
        Future.successful(Left(OrchestrationError.NodeExecutionError(
          node.id,
          node.agent.name,
